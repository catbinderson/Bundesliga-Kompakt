(()=>{
  ...
  async function loadMatches(){
    ...
    for(let attempt=1;attempt<=3;attempt++){
      try{const response=await fetch(API,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const matches=await response.json();return matches.filter(match=>Number(match?.group?.groupOrderID)>=1&&Number(match?.group?.groupOrderID)<=34&&new Date(match.matchDateTime)>=new Date('2026-08-28T00:00:00+02:00'))}catch(reason){error=reason;if(attempt<3)await sleep(400*attempt)}
    }
    throw error;
  }
  ...
  function render(table,finished){
    ...
    return `<div style="display:grid;grid-template-columns:5px 28px 28px minmax(0,1fr) 38px 48px;gap:8px;padding:0 4px 7px;color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.08em"><span></span><span>#</span><span></span><span>Verein</span><span style="text-align:right">TD</span><span style="text-align:right">Pkt</span></div>${rows}<p style="margin:12px 0 0;color:var(--muted);font-size:11px;line-height:1.5">Berechnet aus Vorsaison-Stärke, Aufsteiger-Niveau, Heimvorteil sowie ${finished} bereits beendeten Saisonspielen mit Punkten und Toren. Die Vorhersage wird bei jedem Öffnen aktualisiert und ist keine Garantie.</p>`;
  }
